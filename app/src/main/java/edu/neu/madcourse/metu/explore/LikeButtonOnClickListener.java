...
public class LikeButtonOnClickListener implements View.OnClickListener {
    ...
    public LikeButtonOnClickListener(Button button, RecommendedUser recommendedUser) {
                    ...
                    .addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            for (DataSnapshot dataSnapshot: snapshot.getChildren()) {
                                if (dataSnapshot.getKey().equals(userId + recommendedUser.getUserId())
                                        || dataSnapshot.getKey().equals(recommendedUser.getUserId() + userId)) {
                                    ...
                                    chatItem.setSender(userId);
                                    chatItem.setTimeStamp(System.currentTimeMillis());
                                    chatItem.setMessage(String.format("[%s likes you!]", nickname));
                                    ...
                                    sendNotification(recommendedUser.getUserId()
                                            , String.format("[%s likes you!]", nickname));
                            ...
                            user1.setNickname(App.getNickname());
                            user1.setAvatarUri(App.getAvatarUri());
                                        ...
                                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                                            if (snapshot.exists()) {
                                                ...
                                                try {
                                                    ...
                                                    data.put(Constants.NOTIFICATION_TYPE, Constants.NOTIFY_GET_A_LIKE);
                                                    // put the info for current user
                                                    data.put(Constants.MESSAGE_CONTENT, String.format("[%s likes you]", nickname));
